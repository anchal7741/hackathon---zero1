...
auth.onAuthStateChanged(user => {
    if (user) {
        console.log("logged in");
        db.collection("users").doc(user.uid).collection("Appointments").onSnapshot(snapshot => {
...
const setupGuides = (data) => {
  ...
  data.forEach(doc => {
       ...
       li = 
      ` <li data-aos="fade-up">
      <i class="icofont-calendar icon-help"></i> <a data-toggle="collapse" class="collapse" href="#faq-list-1">${blog.doctor} &nbsp; &nbsp; ${blog.date} &nbsp; &nbsp; ${blog.date}<i class="bx bx-chevron-down icon-show"></i><i class="bx bx-chevron-up icon-close"></i></a>
